...
class BackgroundWorker extends AsyncTask<Void, Void, String> {
    ...
    private int flag = 0, id, appversion, ivmin, ivmax;
    ...
    BackgroundWorker (String page, int ivmin, int ivmax, int id, Context context) {this.page=page; this.ivmax=ivmax; this.ivmin=ivmin; this.id=id; this.context=context; flag=7;}
    ...
    protected String doInBackground(Void... aVoid) {
        switch (flag) {
                    ...
                    case "insertCopy":
                        try {
                            post_data = URLEncoder.encode("attack", "UTF-8")+"="+URLEncoder.encode(attack, "UTF-8")+"&"+
                                        ...
                                        URLEncoder.encode("ivmin", "UTF-8")+"="+URLEncoder.encode(String.valueOf(ivmin), "UTF-8")+"&"+
                                        URLEncoder.encode("ivmax", "UTF-8")+"="+URLEncoder.encode(String.valueOf(ivmax), "UTF-8")+"&"+
            ...
            case 7:
                try {
                    post_data = URLEncoder.encode("ivmin", "UTF-8")+"="+URLEncoder.encode(String.valueOf(ivmin), "UTF-8")+"&"+
                                URLEncoder.encode("ivmax", "UTF-8")+"="+URLEncoder.encode(String.valueOf(ivmax), "UTF-8")+"&"+
                                URLEncoder.encode("pokeID", "UTF-8")+"="+URLEncoder.encode(String.valueOf(id), "UTF-8");
                } catch (UnsupportedEncodingException e) {
                    e.printStackTrace();
                }
                break;
